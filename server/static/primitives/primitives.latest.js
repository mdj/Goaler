/**
 * @preserve Basic Primitives orgDiagram v2.1.1
...
primitives.common.Perimeter.prototype.draw = function (perimeter) {
    ...
    perimeter.segments.iterate(function (item) {
        ...
        if (polyline.segments.length == 0) {
            ...
            connectionHash[perimeter.segments.endSegmentKey] = {
                point: prevPoint,
                isVertical: isVertical
            };
        }
        newPoint = null;
        if (perimeter.segments.segmentsHash.hasOwnProperty(item.oppositeKey)) {
        ...
        if (offset > 0) {
            ...
            if (prevSegment != null) {
                ...
                if (!perimeter.segments.segmentsHash.hasOwnProperty(prevSegment.oppositeKey)
                    && perimeter.segments.segmentsHash.hasOwnProperty(item.oppositeKey)) {
                ...
                } else if (perimeter.segments.segmentsHash.hasOwnProperty(prevSegment.oppositeKey)
                    && !perimeter.segments.segmentsHash.hasOwnProperty(item.oppositeKey)) {
...
primitives.orgdiagram.BaseController.prototype._getNextLevelTreeItem = function (newHighlightItem, isBelow) {
    ...
    for (levelIndex = 0, levelLen = nextLevels.length; levelIndex < levelLen; levelIndex += 1) {
        ...
        for (index = 0, len = treeLevelItems.length; index < len; index += 1) {
            treeItem = this._visualTree(treeLevelItems[index]);
...
primitives.orgdiagram.BaseController.prototype._createVisualTreeItems = function () {
    ...
    this._orgTree.loopPostOrder(this, function (nodeid, node, parentid, parent) {
        ...
        if (logicalParentItem != null) {
            ...
            if (depth > 1) {
                logicalParentItem.visualDepth += (depth - 1);
                if (this._visualTree.hasChildren(logicalParentItem.id)) {
...
primitives.orgdiagram.BaseController.prototype._getLastVisualAggregatorDepth = function (treeItem) {
    ...
    while (treeItem.visualAggregatorId != null) {
        treeItem = this._visualTree.node(treeItem.visualAggregatorId);
        result += 1;
    }
    return result + 1;
};
...
primitives.common.perimeter.LinkedHashItems = function () {
    ...
    this.nextKeys = {};
    this.prevKeys = {};
    this.startSegmentKey = null;
    this.endSegmentKey = null;
};

primitives.common.perimeter.LinkedHashItems.prototype.add = function (key, item) {
    if (this.segmentsHash.hasOwnProperty(key)) {
        throw "Duplicate segments are not supported!";
    }
    this.segmentsHash[key] = item;
    this.nextKeys[key] = null;
    if (this.endSegmentKey == null) {
        this.startSegmentKey = key;
        this.prevKeys[key] = null;
    } else {
        this.nextKeys[this.endSegmentKey] = key;
        this.prevKeys[key] = this.endSegmentKey;
    }
    this.endSegmentKey = key;
};

primitives.common.perimeter.LinkedHashItems.prototype.unshift = function (key, item) {
    if (this.segmentsHash.hasOwnProperty(key)) {
        throw "Duplicate segments are not supported!";
    }
    this.segmentsHash[key] = item;
    this.prevKeys[key] = null;
    if (this.startSegmentKey == null) {
        this.endSegmentKey = key;
        this.nextKeys[key] = null;
    } else {
        this.prevKeys[this.startSegmentKey] = key;
        this.nextKeys[key] = this.startSegmentKey;
    }
    this.startSegmentKey = key;
};

primitives.common.perimeter.LinkedHashItems.prototype.insertAfter = function (afterKey, key, item) {
    if (this.segmentsHash.hasOwnProperty(key)) {
        throw "Duplicate segments are not supported!";
    }
    ...
    if (afterKey == null) {
        ...
        var nextKey = this.nextKeys[afterKey];
        if (nextKey == null) {
            this.add(key, item);
        } else {
            this.segmentsHash[key] = item;
            this.nextKeys[afterKey] = key;
            this.nextKeys[key] = nextKey;
            this.prevKeys[nextKey] = key;
            this.prevKeys[key] = afterKey;
        }
    }
};

primitives.common.perimeter.LinkedHashItems.prototype.remove = function (key) {
    var prevKey = this.prevKeys[key],
        nextKey = this.nextKeys[key];

    if (prevKey != null) {
        this.nextKeys[prevKey] = nextKey;
    } else {
        this.startSegmentKey = nextKey;
    }

    if (nextKey != null) {
        this.prevKeys[nextKey] = prevKey;
    } else {
        this.endSegmentKey = prevKey;
    }

    delete this.segmentsHash[key];
...
primitives.common.perimeter.LinkedHashItems.prototype.iterate = function (onItem, startKey, endKey) {
    this._iterate(true, onItem, startKey, endKey);
};

primitives.common.perimeter.LinkedHashItems.prototype.iterateBack = function (onItem, startKey, endKey) {
    this._iterate(false, onItem, startKey, endKey);
};

primitives.common.perimeter.LinkedHashItems.prototype._iterate = function (forward, onItem, startKey, endKey) {
    var key = startKey,
        segment;

    if (key == null) {
        key = forward ? this.startSegmentKey : this.endSegmentKey;
    }

    if (onItem != null) {
        while (key != null) {
            segment = this.segmentsHash[key];
            if (segment != null) {
                onItem(segment);
            }
            ...
            if (key == endKey) {
                ...
                key = forward ? this.nextKeys[key] : this.prevKeys[key];
            }
        }
    }
};

primitives.common.perimeter.LinkedHashItems.prototype.toArray = function (startKey, endKey) {
    var result = [];

    this.iterate(function (item, startKey, endKey) {
        result.push(item);
    });
...
primitives.common.perimeter.Manager = function (perimeterItems) {
    this.perimetersHash = {}; // hash of permiter items by id
    this.perimetersBySegmentKey = {}; // hash of permiter items by segment id

    if (perimeterItems != null) {
        this.add(perimeterItems);
    }
};

primitives.common.perimeter.Manager.prototype.add = function (perimeterItems) {
    var self = this,
        index, len,
        perimeterItem;
    ...
    for (index = 0, len = perimeterItems.length; index < len; index += 1) {
        ...
        this.perimetersHash[perimeterItem.id] = perimeterItem;

        perimeterItem.segments.iterate(function (item) {
            self.perimetersBySegmentKey[item.key] = perimeterItem;
        }); //ignore jslint
    }
};

primitives.common.perimeter.Manager.prototype.getMergedPerimeters = function (items) {
    ...
    for (index = 0, len = items.length; index < len; index += 1) {
        item = items[index];
        itemsToMerge[item] = !this.perimetersHash.hasOwnProperty(item);
    }
    ...
    for (index = 0, len = items.length; index < len; index += 1) {
        ...
        if (!itemsToMerge[item]) {
            ...
            perimeter = this.perimetersHash[item];
            ...
            cursor = targetPerimeter.segments.startSegmentKey;
            while (cursor != null) {
                cursorSegment = targetPerimeter.segments.segmentsHash[cursor];

                if (this.perimetersBySegmentKey.hasOwnProperty(cursorSegment.oppositeKey)) {
                    adjadjacentPerimeter = this.perimetersBySegmentKey[cursorSegment.oppositeKey];
                ...
                cursor = this._moveCursor(targetPerimeter.segments, cursorSegment);
            }
        }
    }
    return result;
...
primitives.common.perimeter.Manager.prototype._moveCursor = function (segments, cursorSegment) {
    var result = segments.nextKeys[cursorSegment.key],
        nextKey;

    while (cursorSegment != null && cursorSegment.oppositeKey == (nextKey = (result || segments.startSegmentKey))) {

        if (result != null) {
            result = segments.nextKeys[result];
        }

        segments.remove(cursorSegment.key);
        segments.remove(nextKey);

        cursorSegment = segments.segmentsHash[result != null ? segments.prevKeys[result] : segments.endSegmentKey];
    }
...
primitives.common.family = function () {
    ...
    function removeNode(nodeid) {
        var isNullOrEmpty = primitives.common.isEmptyObject;
        if (_nodes[nodeid] != null) {
            _loop(this, _children, nodeid, function (itemid) {
    ...
    function findLargestRoot() {
        ...
        loopTopoReversed(this, function (famItemId, famItem, position) {
            ...
            if (isRoot && famCount[famItemId] > maximum) {
    ...
    function optimizeReferences(onNewBundleItem) {
        var sharedItemsByKey = {},
            sharedItemsById = {},
            currentItems = {},
            nodeid, children, parents, newReferenceItem,
...
primitives.famdiagram.Controller.prototype._virtualCreateOrgTree = function () {
    var index, len, index2, len2, index3, len3, index4, len4,
        famItem,
        ...
        attachedFamilies,
        userItem,
        itemId, childItemId, parentItemId,
        grandParent,
        familyId,
        family, subFamily,
        familyRootItem,
        ...
        children, subChildren, childrenHash,
        parents, parentsHash, parentsCount, key, parentsLevel,
        itemsToOptimize, optimizedItems,
        child, childFamItem, parentItem, childItem,
        self = this,
        parsedId,
        labelConfigsHash,
        labelConfigs, labelConfig,
        itemsHavingSpouses, itemHavingSpouses, spouses, spouses2, spouseid, hasCommonChild, childItemIsCommon,
        orgItemRoot,
        famItemsNonExtracted,
    ...
    if (this.options.items.length > 0) {
        ...
        /* create family of non-extracted items */
        famItemsNonExtracted = this._logicalFamily.clone();
        ...
        grandParent = null;
        familyId = 0;
        while ((grandParent = famItemsNonExtracted.findLargestRoot()) != null) { //ignore jslint
            ...
            */
            this._extractOrgChart(grandParent, this._logicalFamily, famItemsNonExtracted, family);
...
primitives.famdiagram.Controller.prototype._extractOrgChart = function (grandParentId, logicalFamily, famItemsNonExtracted, family) {
    var index, len,
    ...
    famItemsNonExtracted.removeNode(grandParent.id);
    grandParent.familyId = family.id;

    /* extract its children */
    children = this._extractChildren(grandParent, logicalFamily, famItemsNonExtracted, family);

    while (children.length > 0) {
        tempChildren = [];
        for (index = 0, len = children.length; index < len; index+=1) {
            childItem = children[index];
            tempChildren = tempChildren.concat(this._extractChildren(childItem, logicalFamily, famItemsNonExtracted, family));
        }
...
primitives.famdiagram.Controller.prototype._extractChildren = function (parentItem, logicalFamily, famItemsNonExtracted, family) {
    var result = [],
        index, len,
        id, childItem,
        itemByChildrenKey = {},
    ...
    logicalFamily.loopChildren(this, parentItem.id, function (childid, child, levelIndex) {
        if (levelIndex > 0) {
            return logicalFamily.BREAK;
        }
    ...
    if (this.itemByChildrenKey[parentChildrenKey] != null) {
        ...
        logicalFamily.loopChildren(this, parentItem.id, function (childid, child, levelIndex) {
            ...
            family.links.push(new primitives.famdiagram.FamLink(parentItem.id, childid));
        });
    } else {
        ...
        logicalFamily.loopChildren(this, parentItem.id, function (childid, childItem, levelIndex) {
            ...
            if (famItemsNonExtracted.node(childItem.id) != null) {
                children = [];
                logicalFamily.loopChildren(this, childItem.id, function (childid, childItem, levelIndex) {
                    if (levelIndex > 0) {
                        return logicalFamily.BREAK;
                    }
                    children.push(childid);
                ...
                if (itemByChildrenKey[childrenKey] != null) {
                    ...
                    famItemsNonExtracted.removeNode(childItem.id);
